...
samples_from_file <- function (city_file) {
  con <- get_con()
  wkt <- city_file %>% st_read %>% st_geometry %>% st_as_text
  query_ <- paste0(
    "
    SELECT
      *
...
if (T) {
  ...
  for (i in unique(files)) {
    tryCatch({
      ...
      city <- gsub(".*City geojson/|.geojson$", '', i)
      
      message(paste0("Analysing ", city))
      
      saveRDS(df, paste0("eBird data/rds for each city/", city, ".RDS"))
    }, 
    error=function(e){
